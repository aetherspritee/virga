...
def advdiff(
    ...
    if param == "const":
        ...
        mixl = ad_mixl
        z = ad_dz
        qc = (ad_qbelow - ad_qvs) * np.exp(
            -b * fsed / mixl * np.exp(zb / b) * (np.exp(z / b) - 1) + eps * z / b
        )
        advdif = qc + ad_qvs
...
def vfall_aggregrates(r, grav, mw_atmos, t, p, rhop, kf=1.0,D=2.0, Ragg=1.0):
    # FIXME: i dont get this at all, i dont see how this is supposed to work
    """
...
def moment(n, s, loc, scale, dist="lognormal"):
    ...
    def pdf(r, s, loc, scale, dist):
        if dist == "lognormal":
            ...
            return np.exp(-np.log((r - loc) / scale) ** 2 / (2 * s**2)) / (
                r * s * np.sqrt(2.0 * np.pi)
            )
...
def find_rg(rg, fsed, rw, alpha, s, loc=0.0, dist="lognormal"):
    """
