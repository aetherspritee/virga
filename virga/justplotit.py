...
def radii(
    ...
    for j in range(len(out)):
        ...
        for i in gas_name:
            dndr[i] = (
                N[nl, gas_name.index(i)]
                / r
                / np.sqrt(2 * np.pi)
                * np.log(sig)
                * np.exp(
                    -np.log(r / (r_g[nl, gas_name.index(i)] * 1e-4)) ** 2
                    / (2 * np.log(sig) ** 2)
                )
        ...
        for i in gas_name:
            ...
            f = p1.line(
                ...
                source=s1,
                alpha=1,
                color=color[np.mod(indx, len(color))],
                line_width=4,
                legend_label=i,
                line_dash=lines[j],
            )
    ...
    **kwargs : kwargs
    ...
    for j in range(len(out)):
        ...
        if j == 0:
            ...
            kwargs["x_range"] = kwargs.get(
                "x_range",
                [np.max([1e-6, np.min(opd_by_gas * 0.9)]), np.max(opd_by_gas * 1.1)],
            )
            fig = figure(**kwargs)
        ...
        if compare:
            if gas is not None:
                f = fig.line(
                    ...
                    pressure,
                    line_width=4,
                    legend_label=condensibles[indx],
    ...
    **kwargs : kwargs
    ...
    for j in range(len(out)):
        ...
        if j == 0:
            ...
            kwargs["x_range"] = kwargs.get(
                "x_range",
                [np.max([1e-9, np.min(cond_mmr * 0.9)]), np.max(cond_mmr * 1.1)],
            )
            fig = figure(**kwargs)
...
def all_optics(out):
    ...
    if pressure is not None:
        pressure = [
            "{:.1E}".format(i) for i in pressure[::-1]
