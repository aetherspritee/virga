...
def fort_mie_calc(RO, RFR, RFI, THET, JX, R, RE2, TMAG2, WVNO):
    ...
    if JX > IT:
        raise Exception(
            "THE VALUE OF THE ARGUMENT JX IS GREATER THAN IT. PLEASE READ COMMENTS."
        )
    ...
    for J in range(JX):
        ...
        if THETD[J] > 90.0:
            raise Exception(
                " THE VALUE OF THE SCATTERING ANGLE IS GREATER THAN 90.0 DEGREES. PLEASE READ COMMENTS"
            )
    ...
    if IFLAG != 2:
        ...
        FNA = (
            U[7]
            * (U[0] * U[4] * U[6] + K1 * U[0] - DUMSQ * K3 * U[4])
            / (U[1] * U[4] * U[6] + K1 * U[1] - DUMSQ * K3 * U[4])
        )
        FNB = (
            U[7]
            * (U[2] * U[5] * U[6] + K2 * U[2] - DUMSQ * K2 * U[5])
    ...
    while N <= NMX2:
        ...
        CTBRQS += T[1] * (
            TD[0] * TB[0] + TD[1] * TB[1] + TE[0] * TC[0] + TE[1] * TC[1]
        ) + T[3] * (TD[0] * TE[0] + TD[1] * TE[1])
        QEXT += T[2] * (TB[0] + TC[0])
