...
def fractal_calc_qc(
    ...
    def mfp(T, P):
        ...
        n_atmos = P / (K_BOLTZ * T)
        return 1.0 / (np.sqrt(2.0) * n_atmos * PI * d_molecule**2)
    ...
    def visc(T):
        return (
            5.0
            / 16.0
            * np.sqrt(PI * K_BOLTZ * T * (mw_atmos / AVOGADRO))
            / (PI * d_molecule**2)
            / (1.22 * (T / eps_k) ** (-0.16))
        )
    ...
    for i in range(len(z)):
        ...
        T = T_z(z[i])
        P = P_z(z[i])
        qc_out[i] = max([0.0, qt_out[i] - qvs(T, P)])
        mixl_out[i] = mixl(T, P)
    #   --------------------------------------------------------------------
    ...
    for i in range(len(qc_out)):
        if qc_out[i] == 0.0:  # layer is cloud free
            ...
            if analytical_rg:
                ...
                rg[i] = fsed ** (1.0 / alpha) * rw[i] * np.exp(-(alpha + 6) * lnsig2)
                ...
                reff[i] = moment(3, np.log(sig), 0.0, rg[i]) / moment(
                    2, np.log(sig), 0.0, rg[i]
                )
                ...
                ndz[i] = (
                    ...
                    * rho_atmos(T, P)
                    * dz[i]
                    / (4 * np.pi * rho_p * moment(3 + alpha, np.log(sig), 0.0, rg[i]))
                )

        if i > 0:
            qc_path = qc_path + qc_out[i - 1] * (p_out[i - 1] - p_out[i]) / gravity
